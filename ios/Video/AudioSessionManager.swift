...
class AudioSessionManager {
    ...
    private func getCategory(silentSwitchObey: Bool, earpiece: Bool, pip: Bool, needsPlayback: Bool) -> AVAudioSession.Category {
        if needsPlayback {
            if earpiece {
                RCTLogWarn("You can't set \"audioOutput\"=\"earpiece\" and \"ignoreSilentSwitch\"=\"obey\" at the same time (in same or different components) - skipping props")
                return .playback
            }
            
            if silentSwitchObey {
                RCTLogWarn("You can't use \"playInBackground or \"notificationControls with \"ignoreSilentSwitch\"=\"obey\" at the same time (in same or different components) - skipping \"ignoreSilentSwitch prop")
                return .playback
            }
        }
        ...
        if silentSwitchObey {
            ...
            if pip {
                RCTLogWarn("You use \"pictureInPicture\"=\"true\" and \"ignoreSilentSwitch\"=\"obey\" at the same time (in same or different components) - skipping props")
                return .playback
            }
    ...
    func updateAudioSessionCategory() {
        ...
        if anyPlayerNeedsPiP || anyPlayerShowNotificationControls || needUpdateCategory {
            let category = getCategory(silentSwitchObey: isSilentSwitchObey, earpiece: isAnyPlayerUsingEarpiece, pip: anyPlayerNeedsPiP, needsPlayback: canAllowMixing)
