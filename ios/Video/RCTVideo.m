...
- (void)setPaused:(BOOL)paused
{
  if (paused) {
    [_player pause];
    [_player setRate:0.0];
    [self trackFramelessSessionIfNeeded];
  } else {
...
- (void) trackFramelessSessionIfNeeded {
  if (!_frameless) { return; }
  NSMutableDictionary* properties = [[_rotatingVC framelessProperties] mutableCopy];
  if (properties[@"max_degree"] == 0) { return; }
  
  properties[@"video_id"] = _videoId;
  
  [[NSNotificationCenter defaultCenter] postNotificationName:@"RCTVideoFramelessTracking" object:self userInfo:properties];
}
