...
Adjust the control styles using an object with the properties below. These properties apply when `controls={true}`, except for `hideLiveBadge`, which also applies when `controls={false}`.
...
| hideLiveBadge                       | boolean | Hides the LIVE badge drawn over live streams. Default is `false`.   |
