...
`seek()` can only be called after the `onLoad` event has fired. Once completed, the [onSeek](https://thewidlarzgroup.github.io/react-native-video/component/events#onseek) event will be called.
...
This function will change the volume exactly like [volume](https://thewidlarzgroup.github.io/react-native-video/component/props#volume) property. default value and range are the same then.
