...
Sets the DAI (Dynamic Ad Insertion) configuration for server-side ad insertion. DAI supports both Video On Demand (VOD) and Live streaming.

DAI differs from client-side ad insertion (`ad` prop) in that ads are stitched into the stream server-side, providing a seamless viewing experience without playback interruptions.

**Configuration Types:**

There are two types of DAI configurations:

1. **VOD (Video On Demand)** – Requires `contentSourceId` and `videoId`
2. **Live** – Requires `assetKey`
...
| `backupStreamUri` | string | Optional  | Fallback stream URI. If the DAI stream fails to load, the player will automatically fall back to this URI.                                                                                                                |
...
Note: You need to enable IMA SDK in the Gradle or Podfile – see [Enable Client-Side Ads Insertion](/installation). The same IMA SDK extension supports both client-side ads (`ad` prop) and server-side ads (`dai` prop).
